...
## Directories ----
# Point to Box Directory with Data Pooling Data
box <- "/Users/peterbutro/Library/CloudStorage/Box-Box/DatapoolingData"
cleaned_data <- file.path(box,"Data/Analytic/Alcohol")

## Save Locally ----
if(dir.exists("Outputs")){
  message("Directory \'Outputs\' already exists")
  }else{dir.create("Outputs")}

# Load Data ----
# Load in each data source's data
d <- list()
d$hrs <- readRDS(file.path(cleaned_data,"hrs_recoded.RDS"))
d$nlsy<- readRDS(file.path(cleaned_data,"nlsy_recoded.RDS"))
...
nlsy_long <- nlsy_subset  %>%
  ...
  pivot_longer(cols = -c(CASE_ID_NLSY_BL, 
                         ...
                         RACE_ETH_NLSY_BL,
                         MOM_EDU_IND_NLSY_BL, DAD_EDU_IND_NLSY_BL),
               # Collapse vars that share the same prefix & create year variable
               names_to = c(".value", "Year"), 
               names_sep="_NLSY_") %>%  
  mutate(Year = as.numeric(Year)) %>% # set year as numeric
  rename(DAD_EDU = DAD_EDU_NLSY_BL, 
         MOM_EDU = MOM_EDU_NLSY_BL,
         BIRTHYEAR = BIRTHYEAR_NLSY_BL, 
         FEMALE = FEMALE_NLSY_BL, 
         USBIRTH = USBIRTH_NLSY_BL, 
         RACE_ETH = RACE_ETH_NLSY_BL,
         MOM_EDU_IND = MOM_EDU_IND_NLSY_BL, 
         DAD_EDU_IND = DAD_EDU_IND_NLSY_BL)
...
hrs_long <- hrs_subset %>%
  ...
  mutate(
    ...
    RELIGIONCURRENT = RELIGION_HRS_RA,
    MILITARY = MILITARY_HRS_RA,
    BIRTHYEAR = BIRTHYEAR_HRS_RA,
    FEMALE = FEMALE_HRS_RA, 
    USBIRTH = USBIRTH_HRS_RA,
    RACE_ETH = RACE_ETH_HRS_RA,
    MOM_EDU_IND = MOM_EDU_IND_HRS_RA, 
    DAD_EDU_IND = DAD_EDU_IND_HRS_RA
  )
...
hrs_long <- left_join(hrs_long, wave_n)
...
# Choose where you want to save this out to: 
saveRDS(nlsy_long,"Outputs/nlsy_harmonized.RDS")
saveRDS(hrs_long, "Outputs/hrs_harmonized.RDS")
